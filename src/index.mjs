...
export class Authentication{
    ...
    async getChallenge(criteria){
        ...
        const response = await fetch(
            `http://${this.options.host}:${this.options.port}/challenge`,{
                method: 'POST',
                mode: 'cors',
                cache: 'no-cache',
                ...
                if(isBrowser || isJsDom){
                    if(window.PublicKeyCredential){
                        ...
                        if(PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable()){
                            const challengeResult = await this.getChallenge({ username: data.username });
                            const credential = await navigator.credentials.create({
                                publicKey: {
                                    challenge: Uint8Array.from(
                                        challengeResult.challenge, c => c.charCodeAt(0)),
                                    rp: {
                                        name: meta.name,
                                        id: meta.id,
                                    },
                    ...
                    const response = await fetch(
                        `http://${this.options.host}:${this.options.port}/register`, {
                            method: 'POST',
                            mode: 'cors',
                            cache: 'no-cache',
                            credentials: 'same-origin',
            ...
            try{
                if(isBrowser || isJsDom){
                    ...
                    const finishResponse = await fetch(
                        `http://${this.options.host}:${this.options.port}/finish`, {
                            method: 'POST',
                            mode: 'cors',
                            cache: 'no-cache',
                            credentials: 'same-origin',
    ...
    async serve(appInstance){
        ...
        appInstance.post('/register', async (req, res) => {
            ...
            let user = null;
            if( (user = userControl.lookup({ username })) ){
                if(user.registered){
                    res.json({
                        status: 'failed',
                    ...
                    await userControl.remove({ username });
                }
            }
            req.session.challenge = null;
            req.session.user = null;
            user = await userControl.create({ username });
            let credentialRequest = {
                status: 'success',
                challenge: getRandomBytes(32),
                rp: {
        ...
        appInstance.post('/finish', async (req, res)=>{
            try{
                ...
                if(request.username){
                    ...
                    user = await userControl.lookup({username: request.username});
                    if(!user){
                        res.json({
                            status: 'failed',
                ...
                let clientData = JSON.parse(
                    (new Buffer.from(request.clientDataJSON, 'base64')).toString()
                );
                ...
                if(request.attestationObject !== undefined){
                    ...
                    */
                    ...
                    const lookedupUser = userControl.lookup({ username: user.username });
